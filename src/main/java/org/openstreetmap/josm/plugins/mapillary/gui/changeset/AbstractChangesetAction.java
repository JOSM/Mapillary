 ...
 *
 */
public class AbstractChangesetAction extends JosmAction {
  private final MapillaryChangesetDialog changesetDialog;
  private final Supplier<MapillaryChangeset> getChangeset;
  private final Function<MapillaryChangeset, String> changesetEncoder;

  public AbstractChangesetAction(MapillaryChangesetDialog changesetDialog, String text,
    Supplier<MapillaryChangeset> getChangeset, Function<MapillaryChangeset, String> changesetEncoder) {
    super(I18n.tr("Submit deletion changeset"),
      ...
      Shortcut.registerShortcut(String.format("mapillary:submit-%s-changeset", text),
        I18n.tr("Submit current {0} changeset to Mapillary", I18n.tr(text)), KeyEvent.CHAR_UNDEFINED, Shortcut.NONE),
      // CHECKSTYLE.ON: LineLength
      false, "mapillarySubmitChangeset", false);
    this.changesetDialog = changesetDialog;
    setEnabled(false);
    this.getChangeset = getChangeset;
    this.changesetEncoder = changesetEncoder;
  }
  ...
  public void submitChangeset() {
    ...
    if (MapillaryUser.getUsername() == null) {
      ...
      MapillaryChangeset changeset = this.getChangeset.get();
      if (!changeset.checkImages()) {
        ...
        OAuthUtils.addAuthenticationHeader(httpPost);
        String json = this.changesetEncoder.apply(changeset);
            ...
            if (response.getStatusLine().getStatusCode() == 201) {
              try (InputStream inputStream = response.getEntity().getContent();
                ...
                final String message = I18n.trn("{0} image submitted, Changeset key: {1}, State: {2}",
                  "{0} images submitted, Changeset key: {1}, State: {2}", changeset.size(), key, state);
