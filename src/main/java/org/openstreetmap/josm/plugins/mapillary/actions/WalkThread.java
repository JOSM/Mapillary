...
public class WalkThread extends Thread implements Serializable, VectorDataSelectionListener {
    ...
    public void run() {
        try {
            ...
            INode curImage;
            while (!this.endWalk
                && (curSelection = this.data.getSelectedNodes().stream().findFirst().orElse(null)) != null
                ...
                if (!Objects.equals(selectedImage, curSelection)) {
                    // This is just a filler until we the TODO fix ConcurrentModification error is fixed in the
                    // constructor
                    return;
                }
                if (selectedImage != null
                    && selectedImage.getReferrers().stream().filter(IWay.class::isInstance).count() == 1) {
                    final INode nextSelectedImg = MapillarySequenceUtils.getNextOrPrevious(selectedImage,
                        this.goForward);
        ...
        synchronized (cache) {
            try {
                while (cache.get() == null) {
                    ...
                    if (limit < 0) {
                        new Notification(
                            I18n.tr("Walk mode: Waiting for next image takes too long! Exiting walk mode…"))
     ...
     *        next images,
     *        {@link org.openstreetmap.josm.plugins.mapillary.utils.MapillarySequenceUtils.NextOrPrevious#PREVIOUS} if
     *        the
     *        previous images
