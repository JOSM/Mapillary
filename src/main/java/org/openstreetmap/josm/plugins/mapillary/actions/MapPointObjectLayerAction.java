...
public class MapPointObjectLayerAction extends JosmAction {
  ...
  public MapPointObjectLayerAction() {
    super(
      tr(ACTION_NAME), MapillaryPlugin.LOGO.setSize(ImageSizes.DEFAULT), tr(TOOLTIP), Shortcut
    ...
    GuiHelper.runInEDTAndWait(() -> {
      ...
      synchronized (MainApplication.getLayerManager()) {
        ...
        if (MainApplication.getLayerManager().getActiveDataSet() != null
          ...
          MainApplication.getLayerManager().addLayer(new PointObjectLayer(MapillaryKeys.MAPILLARY_POINT_OBJECTS),
            false);
          if (layer != null) {
