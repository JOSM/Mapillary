...
public class ImportMethodDialog extends JDialog {
    ...
    private JPanel getAnswerPanel() {
        ...
        filesBtn.addActionListener(e -> {
            if (JFileChooser.APPROVE_OPTION == IMAGE_FILE_CHOOSER.showOpenDialog(this)) {
                for (File f : IMAGE_FILE_CHOOSER.getSelectedFiles()) {
                    ...
                    final LatLon center = mv == null ? new LatLon(0, 0)
                        : mv.getProjection().eastNorth2latlon(mv.getCenter());
                    try {
                        final VectorWay seq = new VectorWay(MapillaryKeys.IMPORTED_LAYER);
                        seq.setNodes(new ArrayList<>(
                            Utils.filteredCollection(ImageImportUtil.readImagesFrom(f, center), VectorNode.class)));
                        ...
                        JOptionPane.showMessageDialog(this, I18n.tr("Could not import the image ''{0}''!",
                            f.getAbsolutePath(), I18n.tr("Import exception"), JOptionPane.ERROR_MESSAGE));
                    }
                }
            ...
            if (JFileChooser.APPROVE_OPTION == DIRECTORY_CHOOSER.showOpenDialog(this)) {
                ...
                try {
                    ...
                    seq.setNodes(new ArrayList<>(Utils.filteredCollection(
                        ImageImportUtil.readImagesFrom(DIRECTORY_CHOOSER.getSelectedFile(), center),
                        VectorNode.class)));
                    seq.getNodes().forEach(node -> MapillaryLayer.getInstance().getData().addPrimitive(node));
                    if (!MainApplication.getLayerManager().containsLayer(MapillaryLayer.getInstance())) {
                        MainApplication.getLayerManager().addLayer(MapillaryLayer.getInstance());
                    ...
                    JOptionPane.showMessageDialog(this,
                        I18n.tr("Could not import the directory ''{0}''!",
                            DIRECTORY_CHOOSER.getSelectedFile().getAbsolutePath(), I18n.tr("Import exception"),
        ...
        answerPanel.setPreferredSize(
            new Dimension(answerPanel.getPreferredSize().width, answerPanel.getPreferredSize().height * 3));
        return answerPanel;
    }
}
