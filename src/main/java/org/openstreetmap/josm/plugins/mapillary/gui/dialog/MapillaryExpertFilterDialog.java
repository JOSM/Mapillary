...
public class MapillaryExpertFilterDialog extends ToggleDialog implements DataSetListener, MapModeChangeListener {
  ...
  public MapillaryExpertFilterDialog() {
    super(
      tr("Mapillary Expert Filter"), "mapillary-filter", tr("Filter mapillary objects and hide/disable them."), Shortcut.registerShortcut("mapillary:expertfilter", tr("Toggle: {0}", tr("Filter")), KeyEvent.CHAR_UNDEFINED, Shortcut.NONE), 162
    );
    build();
  ...
  private class DeleteAction extends FilterAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      for (int i = selected.length - 1; i >= 0; i--) {
        filterModel.getFilters().get(selected[i]).enable = false;
        filterModel.removeFilter(selected[i]);
      }
  ...
  static class StringRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (model instanceof MapillaryFilterTableModel) {
        cell.setEnabled(((MapillaryFilterTableModel) model).isCellEnabled(row, column));
      }
      return cell;
    }
  }
   ...
   */
  public MapillaryFilterTableModel getFilterModel() {
    return filterModel;
  }
