...
class UVMappingTest {
    ...
    @Test
    void testMapping() {
        assertPointEquals(new Point2D.Double(.5, 1), UVMapping.getTextureCoordinate(new Vector3D(0, 1, 0)),
            DEFAULT_DELTA);
        assertPointEquals(new Point2D.Double(.5, 0), UVMapping.getTextureCoordinate(new Vector3D(0, -1, 0)),
            DEFAULT_DELTA);
        ...
        assertPointEquals(new Point2D.Double(.25, .5), UVMapping.getTextureCoordinate(new Vector3D(-1, 0, 0)),
            DEFAULT_DELTA);
        assertPointEquals(new Point2D.Double(.5, .5), UVMapping.getTextureCoordinate(new Vector3D(0, 0, 1)),
            DEFAULT_DELTA);
        assertPointEquals(new Point2D.Double(.75, .5), UVMapping.getTextureCoordinate(new Vector3D(1, 0, 0)),
            DEFAULT_DELTA);
        assertPointEquals(new Point2D.Double(1, .5), UVMapping.getTextureCoordinate(new Vector3D(0, 0, -1)),
            DEFAULT_DELTA);
        ...
        assertPointEquals(new Point2D.Double(.125, .25),
            UVMapping.getTextureCoordinate(new Vector3D(-.5, -1 / Math.sqrt(2), -.5)), DEFAULT_DELTA);
        assertPointEquals(new Point2D.Double(.625, .75),
            UVMapping.getTextureCoordinate(new Vector3D(.5, 1 / Math.sqrt(2), .5)), DEFAULT_DELTA);
    ...
    private static void assertVectorEquals(final Vector3D expected, final Vector3D actual, final double delta) {
        ...
        assertEquals(expected.getX(), actual.getX(), delta, message);
        assertEquals(expected.getY(), actual.getY(), delta, message);
        assertEquals(expected.getZ(), actual.getZ(), delta, message);
    }
