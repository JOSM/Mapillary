...
THUMB_RE = re.compile(r"thumb_([0-9]+|original)_url")
    ...
    for file in files:
        if ITEM_RE.search(file):
            ...
            request = requests.get(
                MAPILLARY_API_ROOT + file.replace(".json", ""),
                params=params,
                headers=MAPILLARY_HEADERS,
            )
            print(request.url)
